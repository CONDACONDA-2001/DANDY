...
class CBAM(nn.Module):
    def __init__(self, channel, reduction=16):
        super().__init__()
        effective_reduction = max(reduction, channel // 4)  # 确保不会除到0
        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channel, max(1, channel // effective_reduction), 1),  # 至少保持1个通道
            nn.ReLU(inplace=True),
            nn.Conv2d(max(1, channel // effective_reduction), channel, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        # 空间注意力（应基于原始输入）
        sa_input = x  # 使用原始输入
        sa = torch.cat([torch.max(sa_input, 1)[0].unsqueeze(1),
                        torch.mean(sa_input, 1).unsqueeze(1)], dim=1)
...
class FOM(nn.Module):
    ...
    def forward(self, x):
        x0 = self.conv0(x)
        x0 = self.cbam(x0)  # 添加注意力，不影响效率的同时提升模型性能
        x1 = x0 + self.conv1(x0)
        x2 = x1 + self.conv2(x1)
        x3 = x2 + self.conv3(x2)
...
class IGM(nn.Module):
    def __init__(self, channel):
        ...
        self.fom = FOM(channel, channel)
        self.sigmoid_coarse = nn.Sigmoid()
        # 添加通道调整卷积
        self.coarse_conv = nn.Conv2d(1, channel, 1)  # 新增1->channel转换
        # 添加边缘注意力通道转换
        self.edge_conv = nn.Conv2d(1, channel, 1)  # 新增

    def forward(self, c, edge_att, coarse_att):
        # 转换两种注意力到特征图通道
        edge_att = self.edge_conv(edge_att)  # [B,1,H,W] -> [B,C,H,W]
        coarse_att = self.coarse_conv(coarse_att)  # [B,1,H,W] -> [B,C,H,W]
